...
#define  GXWidthRatio(Value)  ((Value)*GXScreenWidthRatio)        // 宽度转化成适配后的float值
#define  GXWidthRatioMax(Value)   (ceil((Value)*GXScreenWidthRatio))  // 宽度转化成适配后的ceil值(不小于float的最小整数)
#define  GXWidthRatioMin(Value)  (floor((Value)*GXScreenWidthRatio)) // 宽度转化成适配后的floor值(不大于float的最大整数)

#define  GXHeightRatio(Value) ((Value)*GXScreenHeightRatio)       // 高度转化成适配后的float值
#define  GXHeightRatioMax(Value)  (ceil((Value)*GXScreenHeightRatio)) // 高度转化成适配后的ceil值(不小于float的最小整数)
#define  GXHeightRatioMin(Value) (floor((Value)*GXScreenHeightRatio))// 高度转化成适配后的floor值(不大于float的最大整数)
